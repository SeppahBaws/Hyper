...
namespace Hyper
{
	TestMesh::TestMesh(RenderContext* pRenderCtx)
	...
	void TestMesh::Bind(const vk::CommandBuffer& cmd) const
	{
		m_pVertexBuffer->Bind(cmd);
		m_pIndexBuffer->Bind(cmd);
	}

	void TestMesh::Draw(const vk::CommandBuffer& cmd) const
	{
		HPR_PROFILE_SCOPE();
